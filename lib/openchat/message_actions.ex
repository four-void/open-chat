defmodule Openchat.MessageActions do
  ...
  def change(user, kind, id, action, params) when kind in [:room, :direct] do
    ...
    result = if Chat.uuid?(id) do
      Repo.transact(fn ->
        with message when not is_nil(message) <- Repo.one(from m in schema, where: m.id == ^id, lock: "FOR UPDATE"),
  ...
  defp changes(user, m, access, :edit, %{"encrypted" => payload}) do
    if m.user_id == user.id and is_nil(m.deleted_at) and access.send and valid_payload?(payload),
      do: {:ok, [data: payload, edited_at: DateTime.utc_now()]}, else: {:error, :forbidden}
  end
  defp changes(user, m, _, :delete, _) do
    if m.user_id == user.id and is_nil(m.deleted_at),
      do: {:ok, [data: %{}, activity: %{}, deleted_at: DateTime.utc_now()]}, else: {:error, :forbidden}
  end
  defp changes(user, m, access, :react, %{"emoji" => emoji, "enabled" => enabled}) when is_boolean(enabled) do
    if is_nil(m.deleted_at) and access.send and emoji in @emojis do
      ...
      if length(ids) <= 1000 do
        reactions = if ids == [], do: Map.delete(reactions, emoji), else: Map.put(reactions, emoji, ids)
  ...
  defp valid_payload?(%{"iv" => iv, "cipher" => cipher, "nonce" => nonce} = p) when map_size(p) == 3 and is_binary(cipher) and byte_size(cipher) <= 16000,
    do: Chat.uuid?(nonce) and Accounts.valid_vault?(%{"iv" => iv, "cipher" => cipher})
  defp valid_payload?(_), do: false

  defp broadcast(:room, m, json), do: OpenchatWeb.Endpoint.broadcast("room:" <> m.room_id, "message", json)
  defp broadcast(:direct, m, json) do
    c = Repo.get!(Openchat.Chat.DirectConversation, m.conversation_id)
    for id <- [c.first_id, c.second_id], do: OpenchatWeb.Endpoint.broadcast("inbox:" <> id, "direct_message", json)
  end
end
